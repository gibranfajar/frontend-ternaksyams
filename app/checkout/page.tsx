...
const Checkout = () => {
    ...
    try {
      const payload = {
        ...
        product: cartItems.map((item: any) => ({
          id: item.sizeId, // pastikan ini = variantsize_id di backend
        })),
      };
  ...
  // Subtotal khusus untuk product yang eligible voucher
  let eligibleSubtotal = 0;

  if (
    voucherData?.voucher_type === "product" &&
    voucherData?.eligible_products
  ) {
    const eligibleIds: number[] = voucherData.eligible_products.map((id: any) =>
      Number(id),
    );

    cartItems.forEach((item: any) => {
      const sizeId = Number(item.sizeId); // 👈 paksa jadi number

      if (eligibleIds.includes(sizeId)) {
        eligibleSubtotal += item.price * item.quantity; // 👈 pakai quantity (bukan qty)
      }
    });
  }
  ...
  } else if (voucherData?.voucher_type === "product") {
    targetAmount = eligibleSubtotal; // 👈 KHUSUS product
  } else {
    // transaction atau lainnya
  ...
  // Diskon tidak boleh melebihi target & tidak boleh negatif
