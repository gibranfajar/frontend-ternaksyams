          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between text-red-600">
                ...
                <span className="font-medium">
                  {transaction.voucher?.type === "shipping"
                    ? transaction.voucher.amount_type === "value"
                      ? (() => {
                          const shippingCost = parseFloat(transaction.cost); // ganti sesuai field kamu
                          const voucherAmount = transaction.voucher.amount;
                          ...
                          return `- ${formatRupiah(discount)}`;
                        })()
                      : `- ${transaction.voucher.amount}%`
                    : "Tidak Ada Diskon Pengiriman"}
                </span>
              </div>
              <div className="flex justify-between text-red-600 pb-3">
                <span>
                  {transaction.voucher?.type === "transaction"
                    ? "Voucher Transaksi"
                    : transaction.voucher?.type === "product"
                      ? "Voucher Produk"
                      : "Voucher / Kupon"}
                </span>
                ...
                <span className="font-medium">
                  {transaction.voucher
                    ? transaction.voucher.type === "transaction" ||
                      transaction.voucher.type === "product"
                      ? transaction.voucher.amount_type === "value"
                        ? `- ${formatRupiah(transaction.voucher.amount)}`
                        : `- ${transaction.voucher.amount}%`
                      : "Tidak Ada Voucher / Kupon"
                    : "Tidak Ada Voucher / Kupon"}
                </span>
              </div>
